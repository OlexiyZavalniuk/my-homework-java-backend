...
public class UpdateUserFunction implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(final APIGatewayProxyRequestEvent input,
        ...
        try {
            String user = input.getPathParameters();
            String requestBody = input.getBody();

            String output = dynamoDBService.updateUser(user, requestBody);
            ...
            return apiGatewayService.getApiGatewayProxyResponseEvent(
                    "An error occurred while executing the lambda function when trying to update user: "
